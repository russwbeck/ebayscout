...
def _norm(s):
    """Mirror buy_rules._normalize_key without importing it."""
...
def test_scenario_a_confirms_and_autos():
    ...
    slogan_years = {"stop stanford": {"1984"}}
...
def test_scenario_a_promotes_lower_ranked_match():
    ...
    out = gr.resolve_with_gemini_slogans(
        crop_candidates, crop_to_slogan, {"stop stanford": {"1984"}}, set(), normalize_fn=_norm)
...
def test_flagged_index_not_auto():
    ...
    flagged_indices = {3}
    out = gr.resolve_with_gemini_slogans(
        crop_candidates, crop_to_slogan, {"stop stanford": {"1984"}}, flagged_indices,
        normalize_fn=_norm)
    assert out[0]["year"] == "1984"
    ...
    out2 = gr.resolve_with_gemini_slogans(
        crop_candidates, crop_to_slogan2, {"stop stanford": {"1984"}}, {99},
        normalize_fn=_norm)
...
def test_scenario_b_disambiguates_by_majority():
    ...
    slogan_years = {"beat pitt": {"1980", "1990"}}
    out = gr.resolve_with_gemini_slogans(
        crop_candidates, crop_to_slogan, slogan_years, set(), normalize_fn=_norm)
...
def test_scenario_b_no_majority_falls_back_to_clip():
    ...
    out = gr.resolve_with_gemini_slogans(
        crop_candidates, crop_to_slogan, {"beat pitt": {"1980", "1990"}}, set(), normalize_fn=_norm)
...
def test_build_multimap_flags_duplicates():
    ...
    assert mm["beat pitt"] == {"1980", "1990"}
    assert mm["stop stanford"] == {"1984"}
