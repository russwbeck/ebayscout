...
# --- Rate-limit retry (ebayscout delta) --------------------------------------
# ebayscout logs PER EVENT (one append per image/confirmation) so a throttle or
# crash never loses already-processed work. The cost is more Sheets writes, which
# can transiently hit the ~60-writes/min quota. This bounded retry absorbs those
# 429/RESOURCE_EXHAUSTED blips so an event's rows are not silently dropped. It is
# the only intentional difference from the verbatim cross-bot copy of this module.

_RATELIMIT_MARKERS = ("429", "resource_exhausted", "rate_limit", "quota", "rate limit")


def _looks_rate_limited(exc) -> bool:
    return any(m in f"{exc}".lower() for m in _RATELIMIT_MARKERS)


def _append_with_retry(append_fn, *, retries=4, base_delay=2.0):
    """Call append_fn(); on a rate-limit error retry with exponential backoff
    (2s, 4s, 8s, 16s). Non-rate errors raise immediately. Final failure raises so
    the caller's fail-open try/except logs it."""
    delay = base_delay
    for attempt in range(retries + 1):
        try:
            append_fn()
            return
        except Exception as exc:   # noqa: BLE001 — re-raised below if not rate-limit
            if attempt >= retries or not _looks_rate_limited(exc):
                raise
            print(f">>> MATCH_LOG: rate-limited (attempt {attempt + 1}/{retries}), "
...
class SheetLogger:
    ...
    def log_image_crops(self, job_id, records):
        ...
        try:
            ...
            _append_with_retry(
                lambda: self._match_ws.append_rows(rows, value_input_option="RAW")
            )
    ...
    def log_confirmation(self, check_id, record):
        ...
        try:
            _append_with_retry(
                lambda: self._confirm_ws.append_row(
