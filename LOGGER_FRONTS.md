...
- **Instrument:** two populations in `confirm_log`, read separately and never pooled. (1) VOLUNTEERED: `correction` / `skip_correction`, written by the per-lot **Any of these wrong?** button (buttonmatcher `handle_lot_corrections_submit`). (2) SAMPLED: `audit_shadow` (what an auto would have written, on a lot the sampler picked) against `audit_hit` / `audit_miss` (how the operator answered it), written by `_hold_auto_for_audit` / `_audit_verdict`. (1) is a floor on the error rate — a wrong auto is only reported if it is noticed; (2) is a rate, over a population a hash chose, and it covers lots under 15 buttons only.
- **Gate:** ~100 auto-confirms with corrections logged → measure precision directly. ≥95% supports widening auto-confirm, e.g. lowering `auto_sort` toward 0.82 (Logger_5 says 0.82 keeps precision ~0.979 at ~3.5× volume — but verify on measured corrections first). Stage D needs ≥98% over ≥300 confirmations. **Close this gate on the SAMPLED cells, not the volunteered ones** — and read what the sample actually covers: small lots. E4 spans both lot shapes, so E4 cannot be closed on it at all.
- **Standing:** **the instrument now exists; it did not before, which is why this front has never moved.** Zero `correction` rows had been logged against ~3,988 auto-path confirmations, and the reason was not discipline: `match_logging` documented `correction`/`skip_correction`, `LOGGING.md` described them, the tracker counted them, and **nothing in either repo wrote them** — the flow this entry told the operator to use was never built. (Verified 2026-09-20 across every caller of `build_confirm_record`, both logging wrappers, and any source literal containing `correction`; the nearest thing that existed was `wrong_slogan`, which is on `/buy`, is `crop_num=1`, and logs `source="slogan_pick"`.) Built 2026-09-20 as SR-05: one **Any of these wrong?** button per lot, listing every auto-confirmed button with what resolved AND what Gemini itself read, taking a typed correction per button. Blank means right; `?` means wrong-but-unknown and logs `skip_correction` without touching the sheet. A correction also swaps the sheet in the lot's own direction. Auto-path precision stays inferred until the first rows land — the difference is that it is now reachable. Status moves BLOCKED → OPEN for that reason; stage stays 1 until there is data. **The second half landed the same day.** Volunteered corrections have a selection bias that gets worse as the auto path widens — the A18 ladder added a second family of no-click confirms, so more buttons than ever are written with nobody looking at them, and an auto nobody looks at is an auto nobody reports. The 1-in-N sample removes that: a hash picks the lot, not the operator's attention. `BUTTONMATCHER_AUDIT_SAMPLE_N=10` by default, 0 to turn it off. It skips lots of 15+ buttons (`BUTTONMATCHER_AUDIT_SAMPLE_MAX_BUTTONS`) because handing back a 100-button lot would cost 100 clicks, so **what this front will measure is small-lot precision** — say so whenever the number is quoted. The sample's rows are bookkeeping and are excluded from every "confirmations" denominator in the workbook; a sampled crop still logs exactly one real confirmation, the operator's.
...
- **Standing:** **SHIPPED on the pipeline path 2026-09-20 — where it had never run at all.** The four rungs were implemented only in `process_grid`; the Gemini pipeline's whole auto gate was `res.get("auto")`, so a crop Gemini never read went to a card however far CLIP's #1 led. Graded by `tools/ladder_replay.py` over the 2026-09-20 export — **325 hand-clicked pipeline buttons, 130 distinct lots, 15 days, out-of-sample** (GAP_ONLY and `slogan_gap` were tuned on Logger_11/12/14 in July): the ladder fires on **90 (27.7%)** and the slogan is right on **90/90** — the one apparent miss was an operator misclick, corrected on the sheet, pipeline right. Per rung: score 40/40, gap_only 22/22, slogan_gap 10/10, ref_combo 18/18. Grading it without the twin guard read 91.6%, because **11 of the 13 apparent errors were edition twins** — right slogan, wrong year, `source=edition_pick`, i.e. the picker demonstrably fired; the live rule is subordinate to `_twin_block` so those fires never happen. One fire was a crop that is **not a button** (`slogan_gap`, overall 0.777, gap 0.150, `ref_sim` 0.841 — a phantom circle that looks like a button to CLIP *and* to the reference check). The detector already knew: `det_gem_unmatched_json` names it. Refusing every unbacked crop costs 18 fires of 90 to catch that one — **17 were real buttons** — so the guard is scoped to `slogan_gap` alone, costing 2. Kill switch `BUTTONMATCHER_PIPELINE_LADDER=0`. Sources `auto_pipeline_<rung>`. **Still open:** the 325 include crops Gemini read and DECLINED, which the live rule skips and no column distinguishes, so 27.7% is an upper bound; watch `PIPELINE_LADDER` lines for `gemini=disagreed`.
- **Prior standing (grid path, 2026-09-07):** **"0 wrong at every rung" does not reproduce.** Regraded on the 2026-09-07 export, split by source per directive 3. On human picks (truth, n=302) the shipped union fires on 169 (56.0%) with **1 wrong** = 99.4%; on `gemini_auto` rows (agreement, n=1,015) it fires on 663 (65.3%) with **14**. The single miss on truth is a look-alike pair — restricted #1 `1992 "Penn State and Proud of it"` at overall 0.892 / gap 0.162 / `slogan_gap` 0.162 against the human's `1992 "'Eers to Penn State"`, source `confusable_pick` — so **the next move is to gate the ladder behind the confusable check, not to loosen a threshold.** The 0.10 step-down stays held: +39 fires for **+3 wrong**. See `SHIPPED_WATCH_REVIEW.md` §4.
...
- **Gate:** **≥98% count agreement with Gemini, PER LOT SHAPE, at real volume — and the shapes ship separately.** Restated 2026-09-20: the pooled gate could not be met while dense lots were averaged in with small ones, and it was never a single question. (a) **Small lots (1–6 buttons):** ≥98% exact agreement, held on a batch it was NOT measured on (§4.2) — one fresh export answers it, since small lots are the bulk of the daily feed. Then the unguided count becomes primary on gated small lots. (b) **Dense lots (7+):** the same ≥98%, but it is 32 points away and will not close by accrual — it needs the fusion work (B3, B19, B25), so dense lots stay on Gemini and this half tracks that work rather than gating the other. (c) **Gated lots where Gemini returns 0** are a Gemini failure, not a counting disagreement; they belong in neither stratum and must be counted and triaged separately, not carried in a denominator. Rollback if disagreement in a SHIPPED stratum exceeds 2% over any 50 lots of that shape. **The tripwire is the `auto_overridden` rate, now read per shape on this tab** — corrected 2026-09-20: this gate said `auto_overridden` "has no UI affordance, so it cannot be the tripwire", and it has had one since 2026-07-19. ✏️ Fix count sits on every `gate=auto` post (`main.py` `_auto_count_blocks`), the modal carries `count_source="auto_overridden"`, and the submit handler downgrades it back to `auto` when the operator opens it and keeps the number. What was missing was a READER, which is what the two new cells are. Read it as a **floor**: the one-tap "Match these N" default lets an unchecked count through as `auto`, so a wrong count nobody looked at is not in the numerator. That is the safe direction for a rollback rule — it can fail to fire, never fire falsely — and the wrong direction for proving a stratum is under 2%, so trip the gate with it, never close it. Instrument the placement blind spot (B22) before flipping.
- **Standing:** **Stage B is not blocked — it is blocked on DENSE lots, and small lots have been clearing the gate the whole time.** Read 2026-09-20 over ~900 pooled images, 331 gated lots, 302 scored: **small lots (1–6) 125/127 = 98.43%**, **dense lots (7+) 112/170 = 65.88%**, pooled 237/302 = **78.48%**. The pooled number was never one question — it was an average of a stratum that passes and a stratum 32 points short, and it hid the first behind the second for as long as the front has existed. Two caveats hold the small-lot ship. **(1) n=127 is thin for a 98% gate:** the 95% Wilson interval is ≈[94.4%, 99.6%] and straddles it, and two more errors would read 96.9% — so the point estimate clears but the true rate is not yet established, which is precisely the §4.2 check. **(2) 5 gated lots carry `gemini_button_count` = 0** and all 5 disagree; they are in the pooled denominator (ISNUMBER(0) is true) and in neither stratum, which is how they surfaced — a lot where Gemini returns nothing is a Gemini failure, not a miscount. Also: 29 of 331 gated lots (8.8%) have no Gemini count at all, well under the corpus-wide 23% (B22), so the gated stratum is better covered than the feed. Earlier readings of this front are not comparable: "1349 gated lots" counted per CROP (fixed `6a62d05`) and the n=215 / 79.6% read predates the pooled corpus. Against the user count the same stratum was 90.7% exact / 99.1% within 1, and the gate still ranks correctly (`suggest` 28.3% vs Gemini, `manual` 5.3%), so `ni_gate` works. Do not flip the user count off. Next: one fresh export for the §4.2 hold on small lots.
