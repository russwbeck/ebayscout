...
- **Fixed 2026-09-12:** both bugs, plus `tests/test_no_unbound_names.py` — a pyflakes sweep for undefined names and unbound locals (it catches both in milliseconds, and `py_compile` catches neither), and two line-order assertions naming this exact defect. Verified the guard fails on the pre-fix code: 3 of 3. **CONFIRMED WORKING 2026-09-12**, on the first pipeline lot after deploy: `>>> PIPELINE: label sidecar written for 69d84a254f6f405f8ce94eb60158f71b (1 circles, base=hough)` — the first sidecar ebayscout has ever written. The first honest reading of this front therefore starts from the deploy date, not from 2026-07-08. It is also where B23's accrual half now lives. See `SHIPPED_WATCH_REVIEW.md` §5.
